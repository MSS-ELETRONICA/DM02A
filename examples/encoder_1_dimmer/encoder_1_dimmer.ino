/*
EXEMPLO: ACIONAMENTO DE APENAS UM MÓDULO DE 2 CANAIS ATRAVÉS DE POTENCIÔMETRO ROTATIVO (ENCODER) COM ATUALIZAÇÃO DO NÍVEL ATUAL PELO FEEDBACK
USO:
  ...
  * POTENCIÔMETRO ROTATIVO (ENCODER)
    ...
    Sentido horário: Aumenta o nível do canal
    Sentido anti-horário: Diminui o nível do canal
...
LIGAÇÕES DO POTENCIÔMETRO ROTATIVO (ENCODER) AO ARDUINO:
  GND: CONECTAR AO GND DO ARDUINO
  +: CONECTAR AO PINO DE SAÍDA DE 5V OU 3.3V
  SW: CONECTAR AO PINO 8 DO ARDUINO
  CLK: CONECTAR AO PINO 7 DO ARDUINO
  DT: CONECTAR AO PINO 6 DO ARDUINO
...
char botao = 8; //Conecte um botão ou o SW do módulo encoder, aciona em nível baixo
char clk = 7;   //Conecte o pino CLK do módulo encoder
char dt = 6;    //COnecte o pino DT do módulo encoder

boolean ultPosicao; //Registra a última posição do encoder
boolean leituraCLK; //Variável para armazenar a leitura do pino CLK

boolean ajustou_encoder = 0;  //Indica se fez ajuste pelo encoder para poder atualizar o nível atual no monitor serial

void setup(){

  pinMode(botao,INPUT_PULLUP);  //Define o pino como entrada ativando o pull up interno
  pinMode(clk,INPUT);
  pinMode(dt,INPUT);
...
void loop(){
  ...
  if(!digitalRead(botao)){//Se o botão for pressionado, troca o canal que está sendo controlado pelo potenciômetro rotativo (encoder)
    delay(50);//Aguarda um tempo e confirma se o botão está realmente pressionado
    if(!digitalRead(botao)){
      canal = !canal; //Inverte o canal que está sendo controlado no momento
      while(!digitalRead(botao)){}//Aguarda o botão ser solto para não trocar de canal mais de uma vez para um mesmo acionamento do botão
    }
  }

  leituraCLK = digitalRead(clk);
  if (leituraCLK != ultPosicao){ //Verifica se mudou a posição do eixo do encoder
    if (digitalRead(dt) != leituraCLK){ //Se o nível lógico do pino DT for diferente do nível lógico do pino CLK, está girando no sentido horário
      if(!canal){
        if(nivel_1 < 70){ //Máximo 70 níveis
          nivel_1 = nivel_1 + 1;//Incrementa 1 unidade
        }
      }else{
        if(nivel_2 < 70){ //Máximo 70 níveis
          nivel_2 = nivel_2 + 1;//Incrementa 1 unidade
        }
      }
    }else{ //Caso contrário, é o sentido anti-horário
      if(!canal){
        if(nivel_1 > 0){
        ...
        if(nivel_2 > 0){
          nivel_2 = nivel_2 - 1;//Decrementa 1 unidade
        }
      }
      
    }
    //Atualiza o nível no dimmer
    if(!canal){//Se estiver em 0 é o canal 1
        dimmer.EnviaNivel(nivel_1,0);
    }else{//Se estiver em 1 é o canal 2
        dimmer.EnviaNivel(nivel_2,1);
    }
    ajustou_encoder = 1;//Indica que fez ajuste de nível pelo potenciômetro rotativo (encoder)
  }
  ultPosicao = leituraCLK; //Atualiza o ultímo nível lógico do pino CLK
  ...
  if(millis() > (last_tempo+2000)){//Verifica se já se passou 500ms, se sim, verifica se houve atualização do nível pelos botões do módulo dimmer ou por chaves externas
    ...
    if(ajustou_encoder){  //Se fez ajuste pelo potenciômetro rotativo (encoder), envia o novo nível para monitor serial
      ajustou_encoder = 0;
      if(!canal){
        Serial.print("Feedback canal 1: ");
